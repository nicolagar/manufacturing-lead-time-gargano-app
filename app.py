...
SAMPLE_ROWS = [
    {"process":"TOP_A","refines":"","predecessor":"NA","successor":"TOP_B","duration":"INHERITED","schedule_rule":"SE","workload":"","workers":"","set_up":""},
    {"process":"TOP_B","refines":"","predecessor":"TOP_A","successor":"STOP","duration":"LT_A","schedule_rule":"SE","workload":100,"workers":20,"set_up":10},
    {"process":"A1","refines":"TOP_A","predecessor":"NA","successor":"A2;A3","duration":50,"schedule_rule":"SE"},
    {"process":"A2","refines":"TOP_A","predecessor":"A1","successor":"A4","duration":"LT_B","schedule_rule":"SE","workers":5},
    {"process":"A3","refines":"TOP_A","predecessor":"A1","successor":"A4","duration":30,"schedule_rule":"SE"},
    {"process":"A4","refines":"TOP_A","predecessor":"A2;A3","successor":"STOP","duration":20,"schedule_rule":"SE"},
]
SAMPLE_FORMULAS = [
...
def normalize_schedule_rule(value) -> str:
    rule = normalize_text(value).upper()
    if rule == "":
        return "SE"
    if rule not in {"SE", "FL"}:
        raise PertDataError(f"Unsupported schedule rule '{value}'. Use SE, FL, or leave blank.")
    return rule
...
def prepare_dataframe(df: pd.DataFrame, formula_map: Dict[str, str]) -> pd.DataFrame:
    ...
    if "schedule_rule_raw" not in df.columns:
        if "schedule_rule" in df.columns:
            df["schedule_rule_raw"] = df["schedule_rule"]
        elif "schedule rule" in df.columns:
            df["schedule_rule_raw"] = df["schedule rule"]
        else:
            df["schedule_rule_raw"] = "SE"
    for col in ["process","refines","predecessor","successor"]:
        df[col] = df[col].map(normalize_text)
    df["schedule_rule"] = df["schedule_rule_raw"].map(normalize_schedule_rule)
    validate_references(df)
    extra_columns = [col for col in df.columns if col not in required + ["schedule_rule_raw", "schedule_rule"]]
...
def dataframe_from_rows(rows: List[Dict], formula_map: Dict[str, str] | None = None) -> pd.DataFrame:
    ...
    if "schedule_rule" not in df.columns and "schedule rule" not in df.columns:
        df["schedule_rule"] = "SE"
    required = ["process","refines","predecessor","successor","duration"]
    missing = [c for c in required if c not in df.columns]
    if missing: raise PertDataError(f"Missing fields: {missing}")
    return prepare_dataframe(df.rename(columns={"duration":"duration_raw", "schedule_rule":"schedule_rule_raw", "schedule rule":"schedule_rule_raw"}), formula_map)
...
def load_pert_excel_from_bytes(content: bytes) -> Tuple[pd.DataFrame, Dict[str, str]]:
    ...
    if has_header:
        ...
        ordered_names = ["process", "refines", "predecessor", "successor", "duration_raw"]
        if "schedule rule" in header_lookup:
            ordered_indices.append(header_lookup["schedule rule"])
            ordered_names.append("schedule_rule_raw")
        elif "schedule_rule" in header_lookup:
            ordered_indices.append(header_lookup["schedule_rule"])
            ordered_names.append("schedule_rule_raw")
        ...
        selected.columns = ordered_names + extra_names
        df = selected
    else:
...
def build_level_graph(level_df: pd.DataFrame) -> nx.DiGraph:
    ...
    for _, row in level_df.iterrows():
        g.add_node(row["process"], duration=float(row["duration"]), schedule_rule=str(row.get("schedule_rule", "SE")))
    for _, row in level_df.iterrows():
...
def compute_level_schedule(level_df: pd.DataFrame, offset: float) -> Tuple[pd.DataFrame, float, nx.DiGraph]:
    ...
    schedule_rule = {n: str(g.nodes[n].get("schedule_rule", "SE")) for n in g.nodes}
    ...
    for n in order:
        ...
        ef[n] = es[n] + dur
        check_value(es[n], n, "earliest_start_forward")
        check_value(ef[n], n, "earliest_finish_forward")
    ...
    for n in order:
        preds = list(g.predecessors(n))
        predecessor_finish_bound = max((scheduled_finish[p] for p in preds), default=0.0)
        desired_start = ls[n] if schedule_rule.get(n, "SE") == "FL" else es[n]
        scheduled_start[n] = max(predecessor_finish_bound, desired_start)
        scheduled_finish[n] = scheduled_start[n] + duration[n]
        check_value(scheduled_start[n], n, "scheduled_start")
        check_value(scheduled_finish[n], n, "scheduled_finish")
        if scheduled_start[n] - ls[n] > 1e-9:
            raise PertDataError(
                f"Schedule rule conflict for process '{n}': requested '{schedule_rule.get(n)}' placement exceeds latest feasible start."
            )
    ...
    for n in order:
        rec = {
            "process": n,
            "schedule_rule": schedule_rule.get(n, "SE"),
            "earliest_start": scheduled_start[n] + offset,
            "earliest_finish": scheduled_finish[n] + offset,
        ...
        check_value(rec["earliest_start"], n, "scheduled_start_offset")
        check_value(rec["earliest_finish"], n, "scheduled_finish_offset")
...
function updateRow(i,f,v){rows[i][f]=v;}function addRow(){const row={process:'',refines:'',predecessor:'NA',successor:'STOP',duration:'',schedule_rule:'SE'};variableColumns.forEach(v=>row[v]='');rows.push(row);renderRows();}function removeRow(i){rows.splice(i,1);if(!rows.length)addRow();else renderRows();}function updateFormula(i,f,v){formulas[i][f]=v;syncVariableColumns();renderAll();}function addFormula(){formulas.push({name:'',formula:''});renderAll();}function removeFormula(i){formulas.splice(i,1);renderAll();}
function rowsForApi(){return rows.filter(r=>r.process||r.refines||r.predecessor||r.successor||r.duration!==''||r.schedule_rule).map(r=>{const o={process:String(r.process||'').trim(),refines:String(r.refines||'').trim(),predecessor:String(r.predecessor||'NA').trim()||'NA',successor:String(r.successor||'STOP').trim()||'STOP',duration:String(r.duration??'').trim(),schedule_rule:String(r.schedule_rule||'SE').trim().toUpperCase()||'SE'};variableColumns.forEach(v=>o[v]=r[v]);return o;});}
...
function renderNetwork(data,shouldFit=true){
  ...
  function cardMarkup(node,row,isCritical){
    ...
    const strokeW=isCritical?2.6:1.2;
  ...
  function compoundMarkup(proc,node,row,isCritical){
    const headerH=node.headerH||30;
    const stroke=isCritical?'#dc2626':(node.depth>1?'#b7c6d8':'#cfd8e3');
    const fill=node.depth>1?'rgba(148,163,184,0.10)':'rgba(148,163,184,0.05)';
    const strokeW=isCritical?2.6:1.4;
    const es=Math.round((row.earliest_start||0)*1000)/1000;
    const ef=Math.round((row.earliest_finish||0)*1000)/1000;
    const dur=Math.round((row.duration||0)*1000)/1000;
  ...
  Object.entries(containers).forEach(([proc,node])=>{
    ...
    svg+=compoundMarkup(proc,node,row,Boolean(row.critical));
  });
  Object.entries(boxes).forEach(([proc,node])=>{
    if(compoundSet.has(proc)) return;
    const row=scheduleMap[proc]||{process:proc};
    svg+=cardMarkup(node,row,Boolean(row.critical));
  });
...
function renderScheduleTable(data){
  ...
  schedule.forEach(r=>{
    html+=`<tr><td>${esc(r.process)}</td><td>${esc(r.refines)}</td><td>${r.depth}</td><td>${esc(r.schedule_rule||'SE')}</td><td>${esc(r.duration_raw)}</td><td>${Math.round((r.duration||0)*1000)/1000}</td><td>${Math.round((r.earliest_start||0)*1000)/1000}</td><td>${Math.round((r.earliest_finish||0)*1000)/1000}</td><td>${Math.round((r.latest_start||0)*1000)/1000}</td><td>${Math.round((r.latest_finish||0)*1000)/1000}</td><td>${Math.round((r.total_float||0)*1000)/1000}</td><td>${r.critical?'Yes':'No'}</td></tr>`;
  });
  html+='</tbody>';
